...
def evaluate_v4_model(model_path, use_tta=False, save_predictions=True):
    ...
    eval_dataset = AIROGSDataset(
        ...
        images_dir=config.EVAL_IMAGES_DIR
    )
    ...
    if save_predictions:
        ...
        predictions_df["predicted_label"] = (predictions > metrics["threshold"]).astype(int)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate V4 model with advanced preprocessing")
    parser.add_argument("model_path", type=str, help="Path to trained V4 model (.keras)")
    parser.add_argument("--tta", action="store_true", help="Enable test-time augmentation")
    parser.add_argument("--no-save", action="store_true", help="Don't save predictions CSV")
    ...
    try:
        ...
        print(f"Sensitivity@95% Specificity: {results['metrics']['sensitivity_at_95_specificity']:.4f}")
