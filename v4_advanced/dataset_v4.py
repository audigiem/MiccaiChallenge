...
class V4AdvancedImageDataGenerator(ImageDataGenerator):
    ...
    def __init__(self, use_od_crop=True, use_clahe=True, use_vessel=False,
                 clahe_clip=2.0, clahe_tile=8, **kwargs):
    ...
    def standardize(self, x):
        ...
        if self.use_od_crop:
            try:
                ...
                if od_info is not None:
                    x = crop_around_optic_disk(x, od_info, crop_factor=config.OD_CROP_FACTOR)
                    # Resize back to target size after cropping
                    x = cv2.resize(x, (config.IMAGE_SIZE, config.IMAGE_SIZE), 
        ...
        if self.use_vessel:
            try:
                from advanced_preprocessing import enhance_vessels, extract_green_channel
    ...
    def print_stats(self):
        ...
        if total > 0:
            ...
            print(f"   OD detected: {stats['od_detected']} ({stats['od_detected']/total*100:.1f}%)")
            print(f"   OD failed: {stats['od_failed']} ({stats['od_failed']/total*100:.1f}%)")
...
def save_preprocessing_samples(dataset, num_samples=10):
    ...
    sample_indices = np.random.choice(len(dataset.train_df), 
                                     min(num_samples, len(dataset.train_df)), 
                                     replace=False)
    ...
    for idx, sample_idx in enumerate(sample_indices):
        ...
        axes[1].set_title(f'V4 Preprocessed (OD={config.USE_OD_DETECTION}, CLAHE={config.USE_CLAHE})')
        axes[1].axis('off')
        
        plt.tight_layout()
        output_path = os.path.join(config.SAMPLES_OUTPUT_DIR, f'sample_{idx+1}_{os.path.basename(image_path)}')
