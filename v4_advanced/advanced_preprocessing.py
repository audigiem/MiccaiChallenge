...
def preprocess_fundus_advanced(image_path, target_size=384, use_od_crop=True, 
                               use_clahe=True, use_vessel_enhance=False):
    """
    ...
    # 3. CLAHE enhancement
    if use_clahe:
        image = apply_advanced_clahe(image, clip_limit=2.0, tile_size=8, color_mode='LAB')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if od_info:
            ...
            cv2.imwrite("optic_disk_detection.jpg", cv2.cvtColor(vis_img, cv2.COLOR_RGB2BGR))
            print("Saved visualization to optic_disk_detection.jpg")
        else:
            print("❌ Optic disk not detected")
        
        print("\nTesting full preprocessing pipeline...")
        processed = preprocess_fundus_advanced(img, target_size=384, use_od_crop=True, 
                                              use_clahe=True, use_vessel_enhance=False)
        
        # Save processed image
        processed_uint8 = (processed * 255).astype(np.uint8)
        cv2.imwrite("preprocessed_advanced.jpg", cv2.cvtColor(processed_uint8, cv2.COLOR_RGB2BGR))
