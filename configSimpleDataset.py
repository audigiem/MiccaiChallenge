...
def main(selected=(0, 1, 4), out_dir="dataset/simple", seed=42, copy_images=True):
    ...
    for idx in selected:
        ...
        if lbl_file is None:
            print(
                f"Warning: label file for dataset {idx} not found, skipping.",
                file=sys.stderr,
            )
            continue
        ...
        df = df.rename(columns={df.columns[0]: "challenge_id", df.columns[1]: "class"})[
            ["challenge_id", "class"]
        ]
        ...
        df["image_path"] = df["image_path"].apply(
            lambda fn: Path("dataset") / str(idx) / fn
        )
    ...
    final_df = (
        pd.concat([rg_all, sampled_nrg], ignore_index=True)
    ...
    print(
        f"Fichiers écrits: {out_csv}  et {len(list(images_out.glob('*')))} images dans {images_out}"
    )
    return 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Génère dataset simple à partir de dataset 0/1/4 (format minimal)"
    )
