...
def train_improved_model():
    ...
    print(f"\n🆕 IMPROVEMENTS:")
    print(f"   Focal Loss: {'ENABLED (γ=' + str(config.FOCAL_LOSS_GAMMA) + ')' if config.USE_FOCAL_LOSS else 'DISABLED'}")
    ...
    if config.USE_FOCAL_LOSS:
        ...
        def weighted_bce(y_true, y_pred):
            weights = tf.where(tf.equal(y_true, 1), config.CLASS_WEIGHTS[1], config.CLASS_WEIGHTS[0])
