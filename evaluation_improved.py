...
def evaluate_with_improvements(
    model_path, data_dir, labels_csv, use_tta=True, use_clahe=False
):
    """
    ...
    sys.stdout.flush()
    dataset = AIROGSDataset(labels_csv=labels_csv, images_dir=data_dir)
    dataset.load_data()
    ...
    for idx, row in test_df.iterrows():
        ...
        # Load image
        img = load_img(
            row["image_path"], target_size=(config.IMAGE_SIZE, config.IMAGE_SIZE)
        )
    ...
    print(
        f"  Improved pAUC:        {pauc:.4f}  {'✅ +' + str(round((pauc - 0.6246) * 100, 2)) + '%' if pauc > 0.6246 else '❌'}"
    )
    print(f"  Baseline Sensitivity: 0.6778")
    print(
        f"  Improved Sensitivity: {sensitivity:.4f}  {'✅ +' + str(round((sensitivity - 0.6778) * 100, 2)) + '%' if sensitivity > 0.6778 else '❌'}"
    )
    print("=" * 70 + "\n")

    # Save results
    results = {
        "improvements": {"tta": use_tta, "clahe": use_clahe},
        "challenge_metrics": {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate model with Week 2 improvements"
    )
    parser.add_argument(
        "--model-path", type=str, required=True, help="Path to trained model (.keras)"
    )
    parser.add_argument(
        "--data-dir", type=str, required=True, help="Directory containing test images"
    )
    parser.add_argument(
        "--labels-csv", type=str, required=True, help="Path to labels CSV file"
    )
