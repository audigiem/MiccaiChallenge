...
class AirogsReportGenerator:
    ...
    def _parse_evaluation_results(self, filepath: Path) -> Dict:
        ...
        for key, pattern in metrics_map.items():
            ...
            if match:
                value = match.group(1)
                results[key] = int(value) if key in ['tp', 'fp', 'tn', 'fn'] else float(value)
            ...
            with open(json_files[0], 'r') as f:
                ...
                if 'challenge_metrics' in tta_data:
                    return {
                        ...
                        'sensitivity_95': tta_data['challenge_metrics']['sensitivity_at_95_spec'],
    ...
    def plot_performance_evolution(self):
        ...
        # Plot 1: pAUC
        ax = axes[0, 0]
        bars = ax.bar(x, pauc_values, color=colors, edgecolor='black', linewidth=1.5, label='Base')
        if tta_indices:
            ax.scatter(tta_indices, tta_pauc, color='red', s=200, marker='*', 
                      label='+ TTA', zorder=10, edgecolor='black', linewidth=2)
        ax.axhline(y=0.70, color='r', linestyle='--', linewidth=2, alpha=0.5, label='Target (0.70)')
        ax.set_xlabel('Model Version', fontsize=13, fontweight='bold')
        ax.set_ylabel('Partial AUC (90-100% spec)', fontsize=13, fontweight='bold')
        ax.set_title('Challenge Metric α: Partial AUC Evolution', fontsize=15, fontweight='bold')
        ax.set_xticks(x)
        ...
        # Plot 2: Sensitivity
        ax = axes[0, 1]
        bars = ax.bar(x, sens_values, color=colors, edgecolor='black', linewidth=1.5, label='Base')
        if tta_indices:
            ax.scatter(tta_indices, tta_sens, color='red', s=200, marker='*',
                      label='+ TTA', zorder=10, edgecolor='black', linewidth=2)
        ax.axhline(y=0.75, color='r', linestyle='--', linewidth=2, alpha=0.5, label='Target (0.75)')
        ax.set_xlabel('Model Version', fontsize=13, fontweight='bold')
        ax.set_ylabel('Sensitivity @ 95% Specificity', fontsize=13, fontweight='bold')
        ax.set_title('Challenge Metric β: Sensitivity Evolution', fontsize=15, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(model_names, rotation=45, ha='right')
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3, axis='y')
        ...
        # Plot 3: AUC-ROC
        ax = axes[1, 0]
        bars = ax.bar(x, auc_values, color=colors, edgecolor='black', linewidth=1.5, label='Base')
        if tta_indices:
            ax.scatter(tta_indices, tta_auc, color='red', s=200, marker='*',
                      label='+ TTA', zorder=10, edgecolor='black', linewidth=2)
        ax.set_xlabel('Model Version', fontsize=13, fontweight='bold')
        ax.set_ylabel('AUC-ROC', fontsize=13, fontweight='bold')
        ax.set_title('Standard Metric: AUC-ROC Evolution', fontsize=15, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(model_names, rotation=45, ha='right')
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3, axis='y')
        ...
        # Plot 4: F1-Score
        ax = axes[1, 1]
        bars = ax.bar(x, f1_values, color=colors, edgecolor='black', linewidth=1.5, label='Base')
        if tta_indices:
            ax.scatter(tta_indices, tta_f1, color='red', s=200, marker='*',
                      label='+ TTA', zorder=10, edgecolor='black', linewidth=2)
        ax.set_xlabel('Model Version', fontsize=13, fontweight='bold')
        ax.set_ylabel('F1-Score', fontsize=13, fontweight='bold')
        ax.set_title('Standard Metric: F1-Score Evolution', fontsize=15, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(model_names, rotation=45, ha='right')
        ax.legend(fontsize=11)
        ax.grid(True, alpha=0.3, axis='y')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'performance_evolution.png', dpi=300, bbox_inches='tight')
        print(f"✅ Saved: performance_evolution.png")
        
    def plot_confusion_analysis(self):
        """Plot confusion matrix analysis as separate figures"""
        # Prepare data
        base_models = [m for m in self.models]
        model_names = [m['name'].replace('Improved ', 'v') for m in base_models]
        ...
        width = 0.35
        bars1 = ax.bar(x - width/2, tp_values, width, label='True Positives',
                      color='#2ecc71', edgecolor='black', linewidth=1.5)
        bars2 = ax.bar(x + width/2, fn_values, width, label='False Negatives',
        ...
        ax.set_title('Positive Case Detection: TP vs FN', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'confusion_tp_fn.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: confusion_tp_fn.png")
        
        # Plot 2: False Positives
        fig, ax = plt.subplots(figsize=(14, 6))
        bars = ax.bar(x, fp_values, color=colors_extended, edgecolor='black', linewidth=1.5)
        ax.set_xlabel('Model Version', fontsize=12, fontweight='bold')
        ax.set_ylabel('False Positive Count', fontsize=12, fontweight='bold')
        ax.set_title('False Positives (out of 11,113 NRG)', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(model_names, rotation=45, ha='right', fontsize=10)
        ax.grid(True, alpha=0.3, axis='y')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'confusion_fp.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: confusion_fp.png")
        
        # Plot 3: Sensitivity trend
        fig, ax = plt.subplots(figsize=(14, 6))
        sensitivities = [tp/(tp+fn) if (tp+fn) > 0 else 0 
                        for tp, fn in zip(tp_values, fn_values)]
        ax.plot(x, sensitivities, marker='o', linewidth=2.5, markersize=10,
        ...
        for i, (xi, sens) in enumerate(zip(x, sensitivities)):
            ax.text(xi, sens + 0.015, f'{sens:.2%}',
                   ha='center', va='bottom', fontsize=9, fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'confusion_sensitivity.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: confusion_sensitivity.png")
    
    def plot_training_curves(self):
        """Plot training curves for all models as separate figures"""
        ...
        for model in models_with_history:
            ...
            if 'loss' in history.columns:
                ax.plot(history['loss'], label=f"{name} (train)", linewidth=2)
            if 'val_loss' in history.columns:
                # Filter out extreme values for better visualization
                val_loss_clean = history['val_loss'].replace([np.inf, -np.inf], np.nan)
                val_loss_clean = val_loss_clean[val_loss_clean < 3]  # Cap at 3 for visibility
                ax.plot(val_loss_clean, label=f"{name} (val)", 
        ...
        for model in models_with_history:
            ...
            auc_col = 'auc' if 'auc' in history.columns else 'val_auc' if 'val_auc' in history.columns else None
            if auc_col:
                ax.plot(history[auc_col], label=name, linewidth=2)
        
        ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
        ax.set_ylabel('AUC', fontsize=12, fontweight='bold')
        ax.set_title('Validation AUC Progression', fontsize=14, fontweight='bold')
        ax.legend(fontsize=9, loc='best')
        ax.grid(True, alpha=0.3)
        ax.set_ylim([0.7, 1.0])
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'training_auc.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: training_auc.png")
        ...
        for model in models_with_history:
            ...
            if 'precision' in history.columns:
                ax.plot(history['precision'], label=f"{name} (train)", linewidth=2)
            if 'val_precision' in history.columns:
                ax.plot(history['val_precision'], label=f"{name} (val)",
                       linewidth=2, linestyle='--')
        
        ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
        ax.set_ylabel('Precision', fontsize=12, fontweight='bold')
        ax.set_title('Training & Validation Precision', fontsize=14, fontweight='bold')
        ax.legend(fontsize=9, loc='best')
        ax.grid(True, alpha=0.3)
        ax.set_ylim([0, 1.0])
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'training_precision.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: training_precision.png")
        ...
        for model in models_with_history:
            ...
            if 'recall' in history.columns:
                ax.plot(history['recall'], label=f"{name} (train)", linewidth=2)
            if 'val_recall' in history.columns:
                ax.plot(history['val_recall'], label=f"{name} (val)",
                       linewidth=2, linestyle='--')
        
        ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
        ax.set_ylabel('Recall', fontsize=12, fontweight='bold')
        ax.set_title('Training & Validation Recall', fontsize=14, fontweight='bold')
        ax.legend(fontsize=9, loc='best')
        ax.grid(True, alpha=0.3)
        ax.set_ylim([0, 1.0])
        
        plt.tight_layout()
        plt.savefig(self.output_dir / 'training_recall.png', dpi=300, bbox_inches='tight')
        plt.close()
        print(f"✅ Saved: training_recall.png")
    ...
    def copy_existing_figures(self):
        ...
        for model in self.models:
            ...
            name_clean = model['name'].replace(' ', '_').replace('(', '').replace(')', '')
