"""
Week 2 Training Script with Improvements - OPTIMIZED V2
Fixes issues from previous training:
- Model not learning initially (0 precision/recall)
- Over-regularization preventing convergence
...
import config_improved_v2 as config
...
def create_improved_generators(dataset, batch_size=32):
    """Create data generators with CLAHE preprocessing and moderate augmentation"""
    ...
    # Test generator (minimal preprocessing)
...
def train_model():
    ...
    # Setup
    print("\n" + "=" * 70)
    print("AIROGS GLAUCOMA DETECTION - OPTIMIZED IMPROVED TRAINING V2")
    print("=" * 70)
    ...
    print(f"   Epochs: {config.EPOCHS} (REDUCED from 30)")
    print(f"   Learning rate: {config.LEARNING_RATE} (INCREASED from 5e-5)")
    print(f"   Focal loss: gamma={config.FOCAL_LOSS_GAMMA} (REDUCED), alpha={config.FOCAL_LOSS_ALPHA} (INCREASED)")
    print(f"   CLAHE preprocessing: {config.USE_CLAHE}")
    print(f"   LR Warmup: {config.USE_LR_WARMUP} ({config.WARMUP_EPOCHS} epochs)")
    print(f"   Progressive unfreezing: epoch {config.UNFREEZE_AT_EPOCH}")
    ...
    val_counts = dataset.val_df["label"].value_counts()
    ...
    if config.USE_FOCAL_LOSS:
        ...
        print(f"   Using Focal Loss (gamma={config.FOCAL_LOSS_GAMMA}, alpha={config.FOCAL_LOSS_ALPHA})")
    else:
        # Weighted BCE
        ...
        print(f"   Using Weighted BCE (weights={config.CLASS_WEIGHTS})")
    ...
    model_name = f"airogs_improved_v2_{config.MODEL_BACKBONE}_{timestamp}"
